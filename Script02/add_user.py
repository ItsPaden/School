...
def add_user(username, id, office, phone, dept, group): #user add options
    shell = ""
    if group == "office":
        shell = "/bin/bash" #"/bin/csh" cannot find csh on my system so changing to bash for testing but logic works none the less
    else:
        shell = "/bin/bash"

    #User add with csv info
    try:
        ...
        os.system("sudo passwd --expire " + username) #expire account right away
    except:
...
for row in rows:
    ...
    #Test cases for invalid info, could also iterate through each row if on a larger scale
    if (row[2] == ""):
